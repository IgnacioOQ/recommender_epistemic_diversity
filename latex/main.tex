...
posterior gap \,($\delta = 0.05$) & $0.48$ & $0.47$ & $0.55$ & $0.65$ & $0.79$ & $0.96$\\
\bottomrule
\end{tabular}
\end{center}

Second, the prior. Holding $\Delta = 0.01$ and $T = 200$ and instead concentrating the shared prior to $\mathrm{Beta}(s,s)$ gives the second table, and the two criteria part company. The posterior-gap probability collapses as $s$ grows: a tight prior pins both posteriors near $\tfrac12$, the belief gap rarely clears $\delta$, and $\hat q_{\text{gap}}$ crosses the target at $s \approx 55$, that is, a $\mathrm{Beta}(55,55)$ prior. The last-$k$ probability barely moves, sitting near $\tfrac12$ throughout: it records which arm is pulled, not how far beliefs have separated, and with the arms this close which arm the agent settles on is essentially a coin flip whatever $s$ is. So the practical knob for a target $q$ under the posterior-gap criterion is the prior concentration (or, equivalently, the horizon), not the gap; and the two criteria, asymptotically equivalent as $T \to \infty$, sit far apart in this finite-horizon, strong-prior regime. Making last-$k$ itself rare would instead require shortening the horizon toward $k$.

\begin{center}
\small
\begin{tabular}{lcccccc}
\toprule
\multicolumn{7}{c}{\textbf{Prior sweep} \quad (fixed $\Delta = 0.01$, $T = 200$, $n = 1500$; notebook 1c)}\\
\midrule
$s$ & $1$ & $10$ & $50$ & $100$ & $150$ & $200$\\
\midrule
last-$k$ \,($k = 20$) & $0.53$ & $0.51$ & $0.50$ & $0.50$ & $0.49$ & $0.48$\\
posterior gap \,($\delta = 0.05$) & $0.49$ & $0.33$ & $0.06$ & $0.01$ & $0.00$ & $0.00$\\
\bottomrule
\end{tabular}
\end{center}

\paragraph{The community baseline.}
To fix ideas, suppose $q = 0.05$, which (by the prior sweep) the posterior-gap criterion reaches at this gap and horizon once the shared prior is concentrated to about $\mathrm{Beta}(55,55)$. In that regime no individual agent is at all likely to succeed. Because the agents are independent, the probability that the community succeeds is nonetheless much higher than the probability of individual success. With $n = 100$ agents,
\[
